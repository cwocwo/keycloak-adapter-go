...
import (
	...
	keycloak "github.com/euforia/keycloak-client"
)

var (
	testCfgFile = "./etc/keycloak-config-sample.json"
	testConfig  *keycloak.ClientConfig
	//testKCClient         *Client
...
func Test_LoadClientConfig(t *testing.T) {
	var err error
	if testConfig, err = keycloak.LoadClientConfig(testCfgFile); err != nil {
		t.Fatal(err)
	}
...
func Test_GetResourceRoles(t *testing.T) {
	...
	roles, err := testConfig.GetResourceRoles(keycloak.NewKeycloakJWT(j))
	if err != nil {
		t.Fatal(err)
	}
...
func Test_GetRealmRoles(t *testing.T) {
	j, _ := jws.ParseJWT([]byte(testExpiredAuthToken))
	roles, err := keycloak.NewKeycloakJWT(j).GetRealmRoles()
	if err != nil {
		t.Fatal(err)
	}
	t.Logf("Role (realm): %#v\n", roles)
